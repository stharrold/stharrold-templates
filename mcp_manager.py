...
def validate_credentials() -> dict[str, bool]:
    ...
    common_env_vars = ["GITHUB_TOKEN", "AZURE_DEVOPS_PAT", "OPENAI_API_KEY", "ANTHROPIC_API_KEY"]
        ...
        if "mcpServers" in self.data:
            for name, config in self.data["mcpServers"].items():
                if not name.startswith("DISABLED_"):
                    self.servers.append({"name": name, "config": config, "scope": "global", "project": None})
            ...
            for project_path, project_config in self.data["projects"].items():
                if "mcpServers" in project_config:
                    for name, config in project_config["mcpServers"].items():
                        if not name.startswith("DISABLED_"):
                            self.servers.append({"name": name, "config": config, "scope": "project", "project": project_path})
            ...
            if "projects" in self.data and project_path in self.data["projects"]:
                if "mcpServers" in self.data["projects"][project_path]:
                    if server_name in self.data["projects"][project_path]["mcpServers"]:
                        ...
                        self.data["projects"][project_path]["mcpServers"][disabled_name] = self.data["projects"][project_path]["mcpServers"].pop(server_name)
                        disabled = True
        else:
            ...
            if "projects" in self.data and project_path in self.data["projects"]:
                if "mcpServers" in self.data["projects"][project_path]:
                    if disabled_server_name in self.data["projects"][project_path]["mcpServers"]:
                        ...
                        self.data["projects"][project_path]["mcpServers"][original_name] = self.data["projects"][project_path]["mcpServers"].pop(disabled_server_name)
                        enabled = True
        else:
...
class MCPManager:
    ...
    def interactive_add(self, config: MCPConfig) -> None:
        ...
        if server_type == "1":
            ...
            server_config = {"type": "stdio", "command": "npx", "args": [package] + (args.split() if args else [])}
        elif server_type == "2":
            command = input("Command: ").strip()
            args = input("Arguments (optional): ").strip()

            server_config = {"type": "stdio", "command": command, "args": args.split() if args else []}
        elif server_type == "3":
            url = input("SSE URL: ").strip()

            server_config = {"type": "sse", "url": url}
        else:
            print("Invalid choice.")
            return
...
Examples:
  ...
  mcp_manager.py --backup-only             # Create backups only
    ...
    parser.add_argument("--file", type=Path, help="Work with specific config file only (for compatibility)")
    parser.add_argument("--list", action="store_true", help="List servers only, no removal options")
    parser.add_argument("--remove", action="store_true", help="Start interactive removal process")
    parser.add_argument("--disable", action="store_true", help="Start interactive disable process")
    parser.add_argument("--enable", action="store_true", help="Start interactive enable process")
    parser.add_argument("--backup-only", action="store_true", help="Create backups without making changes")
    parser.add_argument("--check-credentials", action="store_true", help="Validate credential configuration")
    parser.add_argument("--add", action="store_true", help="Interactive server addition")

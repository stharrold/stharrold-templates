...
4. A new `/3_implement` command automates task execution from tasks.md
5. The `/workflow` command is renamed to `/4_deploy` to handle PR deployment
...
2. **Given** I invoke `/0_specify`, **When** I view its description, **Then** it shows navigation: "(start) → workflow/0_specify → workflow/1_plan"

3. **Given** I invoke `/1_plan`, **When** I view its description, **Then** it shows navigation: "workflow/0_specify → workflow/1_plan → workflow/2_tasks"

4. **Given** I invoke `/2_tasks`, **When** I view its description, **Then** it shows navigation: "workflow/1_plan → workflow/2_tasks → workflow/3_implement"

5. **Given** I invoke `/3_implement` (NEW), **When** I view its description, **Then** it shows navigation: "workflow/2_tasks → workflow/3_implement → workflow/4_deploy"

6. **Given** the `/3_implement` command is run, **When** tasks.md exists, **Then** it:
...
7. **Given** I invoke `/4_deploy` (previously `/workflow`), **When** I view its description, **Then** it shows navigation: "workflow/3_implement → workflow/4_deploy → (end)"

8. **Given** the `/4_deploy` command is run, **When** following the full deployment flow, **Then** it executes: contrib PR → develop → release PR → main; release PR → develop; contrib rebase on develop

### Edge Cases
- What happens when user invokes old `/workflow` command? → Command no longer exists, user informed of new `/4_deploy` name
- What happens when user invokes old `/specify`, `/plan`, `/tasks` commands? → Commands no longer exist, user informed of new `/0_specify`, `/1_plan`, `/2_tasks` names
- How does system handle documentation references to old command names? → CLAUDE.md and related docs must be updated
- What if `/3_implement` is run without tasks.md? → Error with guidance to run `/2_tasks` first
- What if tasks fail during `/3_implement`? → User can stop, fix, and resume; progress is tracked
...
- **FR-002**: System MUST rename existing commands with numeric order prefixes:
  - `specify.md` → `workflow/0_specify.md` (command: `/0_specify`)
  - `plan.md` → `workflow/1_plan.md` (command: `/1_plan`)
  - `tasks.md` → `workflow/2_tasks.md` (command: `/2_tasks`)
  - `workflow.md` → `workflow/4_deploy.md` (command: `/4_deploy`)
- **FR-003**: System MUST create NEW command `workflow/3_implement.md` (command: `/3_implement`) that:
  - Loads tasks from `specs/{feature}/tasks.md`
  - Executes tasks automatically in dependency order
  - Uses TodoWrite to track progress
  - Supports parallel execution for [P] marked tasks
  - Runs quality gates before completion
  - Errors if tasks.md doesn't exist (guides user to run `/2_tasks` first)
...
- **FR-006**: The `/4_deploy` command MUST handle the complete deployment flow:
...
- **FR-008**: System MUST update all internal references to use new command names (`/0_specify`, `/1_plan`, `/2_tasks`, `/3_implement`, `/4_deploy`)
...
- Creating 1 new slash command (`/3_implement`)
...
- Modifying underlying skill scripts (beyond what /3_implement needs)
...
Per Claude Code documentation: Subdirectories create organizational namespaces that appear in help output as "(project:subdirectory)" but do not change the command invocation syntax. Commands are invoked by filename: `/0_specify`, `/1_plan`, `/2_tasks`, `/3_implement`, `/4_deploy`. The numeric prefixes ensure clear execution order.

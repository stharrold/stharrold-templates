...
1. **Given** the existing `.gemini/commands/` structure, **When** I list available commands with `/help`, **Then** all workflow commands show "(project:workflow)" organization tag
...
6. **Given** the `/4_implement` command is run, **When** tasks.md exists, **Then** it:
   ...
   - User can stop/rewind via Gemini Code controls
...
- How does system handle documentation references to old command names? → GEMINI.md and related docs must be updated
...
- **FR-007**: System MUST update GEMINI.md to reflect new command names and navigation
...
- **Slash Command File**: Markdown file defining a Gemini Code command, containing frontmatter (description, order, prev, next) and execution instructions
- **Namespace Subdirectory**: Folder under `.gemini/commands/` that groups related commands and appears as "(project:workflow)" in help output
...
- Updating GEMINI.md workflow documentation
...
Per Gemini Code documentation: Subdirectories create organizational namespaces that appear in help output as "(project:subdirectory)" but do not change the command invocation syntax. Commands are invoked by filename: `/1_specify`, `/2_plan`, `/3_tasks`, `/4_implement`, `/5_integrate`. The numeric prefixes ensure clear execution order.

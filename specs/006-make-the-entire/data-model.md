...
| `state_dir` | `Path` | Path to `.gemini-state/` directory |
...
- `state_dir` = `worktree_root / ".gemini-state"`
...
| Field | Type | Description |
|-------|------|-------------|
| `path` | `Path` | `.gemini-state/` absolute path |
...
JSON structure stored in `.gemini-state/workflow.json`.
...
- Workflow progress: Stored in `.gemini-state/workflow.json`

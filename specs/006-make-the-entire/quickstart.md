...
- Git repository with `.gemini/skills/` structure
...
state_dir = Path(root) / '.gemini-state'
...
**Expected Output**:
```
State directory: /workspace/.gemini-state
...
podman-compose run --rm dev python .gemini/skills/git-workflow-manager/scripts/create_worktree.py feature test-worktree contrib/stharrold
...
- [ ] `.gemini-state/` directory created automatically

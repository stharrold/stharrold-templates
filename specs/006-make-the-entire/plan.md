...
Enable multiple Gemini Code instances to work on different features concurrently in separate git worktrees without state conflicts. The solution creates a `.gemini-state/` directory within each worktree to store mutable workflow state (DuckDB, workflow progress) while keeping skill definitions (`.gemini/skills/`) shared and read-only.
...
- Library docs: Updated GEMINI.md

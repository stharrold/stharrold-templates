...
### Primary User Story
As a developer running multiple Claude Code instances concurrently, I want each instance working in a separate git worktree to have completely isolated state and context, so that multiple features can be developed in parallel without conflicts or interference.
...
2. **Given** two Claude Code instances running in separate worktrees, **When** both instances execute `/workflow/all`, **Then** each instance tracks only its own feature's progress without affecting the other.
...
5. **Given** shared skill definitions in `.claude/skills/`, **When** accessed from any worktree, **Then** the skill code is read-only and shared, but any state the skill generates is worktree-specific.
...
- **FR-003**: System MUST allow skill definitions (`.claude/skills/`) to remain shared and read-only across all worktrees.
...
- **FR-010**: System MUST prevent workflow state conflicts when multiple Claude Code instances run concurrently in different worktrees.
...
- **Shared Skills**: Read-only skill definitions in `.claude/skills/` that are shared across all worktrees without duplication.

- **Worktree State Directory**: A per-worktree location (e.g., `.claude-state/` within each worktree) where all mutable state is stored.

...
### File: `.claude/skills/agentdb-state-manager/scripts/init_database.py`
...
### File: `.claude/skills/git-workflow-manager/scripts/worktree_agent_integration.py`
...
**Required Changes**:
1. Optionally initialize `.claude-state/` in new worktree
...
### File: `.claude/skills/workflow-utilities/scripts/todo_updater.py`
...
- Workflow progress (mutable state) moves to `.claude-state/workflow.json`

### New File: `.claude/skills/workflow-utilities/scripts/workflow_progress.py`
...
### File: `.claude/skills/quality-enforcer/scripts/run_quality_gates.py`
...
1. **Non-worktree repos**: `is_worktree=False`, state in repo root `.claude-state/`
...
**Verify**:
- Each has separate `.claude-state/` directory

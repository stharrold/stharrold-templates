...
**Date**: 2025-11-23
**Location**: `.claude/skills/workflow-utilities/scripts/worktree_context.py`
...
**Returns**: `Path` to `.claude-state/` directory

**Behavior**:
1. Call `get_worktree_context()`
2. Construct path: `{worktree_root}/.claude-state/`
...
**Then**:
- Directory is created at `.claude-state/`
- `.gitignore` contains `*`
...
**Given**: A worktree was removed but `.claude-state/` remains

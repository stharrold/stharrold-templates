...
**Key Files**:
- Schema: `.claude/skills/agentdb-state-manager/schemas/agentdb_sync_schema.sql`
- Engine: `.claude/skills/agentdb-state-manager/scripts/sync_engine.py`
- Init: `.claude/skills/agentdb-state-manager/scripts/init_database.py`
...
**Key Files**:
- Updater: `.claude/skills/workflow-utilities/scripts/todo_updater.py`
- Registrar: `.claude/skills/workflow-utilities/scripts/workflow_registrar.py`
- Archiver: `.claude/skills/workflow-utilities/scripts/workflow_archiver.py`
...
**Key Files**:
- Creator: `.claude/skills/git-workflow-manager/scripts/create_worktree.py`
- Integration: `.claude/skills/git-workflow-manager/scripts/worktree_agent_integration.py`
...
| Per-Worktree | None | `.claude-state/` directory |
...
**Chosen**: `.claude-state/` directory within each worktree

**Rationale**:
- Mirrors `.claude/` for skills (read-only) vs `.claude-state/` for state (read-write)
...
**Chosen**: Skills remain shared (`.claude/skills/`), only state is worktree-local
...
├── .claude-state/           # Worktree-specific state
...
├── .claude/skills/          # -> symlink or accessed from main
├── .claude-state/           # Separate state directory

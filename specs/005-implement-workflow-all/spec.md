...
1. **Given** I'm on a feature branch with a spec, **When** I run `/workflow/all`, **Then** it should execute `/2_plan`, `/3_tasks`, `/4_implement`, `/5_integrate` in sequence, pausing after `/5_integrate` since PR merges are manual.

2. **Given** I'm starting a new feature, **When** I run `/workflow/all new "feature description"`, **Then** it should start from `/1_specify` and continue through the workflow.

3. **Given** I'm on `develop` branch after merging, **When** I run `/workflow/all release`, **Then** it should execute `/6_release` and pause before `/7_backmerge` for PR merge.
...
- **FR-005**: System MUST support `new "description"` mode to start fresh from `/1_specify`
- **FR-006**: System MUST support `release` mode to run `/6_release` → `/7_backmerge`
...
- **ManualGate**: Points where workflow pauses (after /5_integrate PR creation, after /6_release PR creation, after /7_backmerge pr-develop step)

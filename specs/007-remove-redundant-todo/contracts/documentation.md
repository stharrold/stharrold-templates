...
| 6. AI Config Sync | GEMINI.md → AGENTS.md synced |
```
...
podman-compose run --rm dev python .gemini/skills/git-workflow-manager/scripts/pr_workflow.py archive-todo

# Step 3: Sync GEMINI.md → AGENTS.md
...
```
...
**Requirement**: Must be synchronized copy of GEMINI.md

**Validation**: After GEMINI.md update, run sync script:
```bash
podman-compose run --rm dev python .gemini/skills/git-workflow-manager/scripts/pr_workflow.py sync-agents
```
...
### Test: No TODO references in GEMINI.md
```python
def test_gemini_md_no_todo_refs():
    """GEMINI.md should not reference TODO*.md system."""
...
```python
def test_gemini_md_five_gates():
    """GEMINI.md quality gates table should have 5 gates."""
    content = Path('GEMINI.md').read_text()
...
def test_agents_md_synced():
    """AGENTS.md should match GEMINI.md content."""

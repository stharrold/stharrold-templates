...
- `.claude/skills/workflow-orchestrator/templates/TODO_template.md` - template, preserve
...
1. **Quality gate script** - `.claude/skills/quality-enforcer/scripts/run_quality_gates.py`
...
2. **PR workflow script** - `.claude/skills/git-workflow-manager/scripts/pr_workflow.py`
   - Has `archive-todo` step in workflow
   - **Action**: Remove or skip this step

3. **TODO updater script** - `.claude/skills/workflow-utilities/scripts/todo_updater.py`
   - Dedicated script for TODO file management
   - **Action**: Deprecate (move to archived or delete)

4. **Sync TODO to DB** - `.claude/skills/agentdb-state-manager/scripts/sync_todo_to_db.py`
   - Syncs TODO*.md content to DuckDB
   - **Action**: Deprecate or make optional

5. **Create worktree script** - `.claude/skills/git-workflow-manager/scripts/create_worktree.py`
...
**Decision**: Update CLAUDE.md, AGENTS.md, and related docs

**Findings**:
- `CLAUDE.md` - Quality gates table mentions "TODO Frontmatter" gate
- `CLAUDE.md` - "TODO*.md YAML Frontmatter (Required)" section
- `CLAUDE.md` - PR workflow mentions `archive-todo` step

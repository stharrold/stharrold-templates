...
- [ ] T002 Remove `check_todo_frontmatter()` function from `.gemini/skills/quality-enforcer/scripts/run_quality_gates.py`
  - Delete the entire function (~50 lines)
  - Remove function call in `run_all_gates()`
  - Remove from imports if separate

- [ ] T003 Update gate numbering in `.gemini/skills/quality-enforcer/scripts/run_quality_gates.py`
  - Change `[5/6]` to `[5/5]` for AI Config Sync gate
  - Change `[X/6]` to `[X/5]` for all gates (1-5)
  - Update any gate count variables

- [ ] T004 Remove `todo_frontmatter` from results dict in `.gemini/skills/quality-enforcer/scripts/run_quality_gates.py`
...
- [ ] T005 Update quality gates table in `GEMINI.md`
...
- [ ] T008 Remove TODO reference from critical guidelines in `GEMINI.md`
...
- [ ] T009 Sync `AGENTS.md` from `GEMINI.md`
  ```bash
...
- [ ] T011 [P] Verify no TODO references remain in documentation
  ```bash
  grep -rn "TODO Frontmatter" GEMINI.md AGENTS.md
  grep -rn "archive-todo" GEMINI.md AGENTS.md
  ```
  - Expected: No matches
...
- T009 requires T005-T008 complete (syncs final GEMINI.md)

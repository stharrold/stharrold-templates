...
- [ ] T001 [P] Create `record_sync.py` in `.gemini/skills/agentdb-state-manager/scripts/`
...
- [ ] T002 [P] Create `query_workflow_state.py` in `.gemini/skills/agentdb-state-manager/scripts/`
...
- [ ] T003 [P] Add `--no-todo` flag to `.gemini/skills/git-workflow-manager/scripts/create_worktree.py`
...
- [ ] T004 [P] Add `--no-archive` flag to `.gemini/skills/git-workflow-manager/scripts/cleanup_feature.py`
...
- [ ] T005 [P] Add `--issue` flag to `.gemini/skills/speckit-author/scripts/create_specifications.py`
...
- [ ] T006 Rewrite `/1_specify` in `.gemini/commands/workflow/1_specify.md`
...
- [ ] T007 Rewrite `/2_plan` in `.gemini/commands/workflow/2_plan.md`
...
- [ ] T008 Rewrite `/3_tasks` in `.gemini/commands/workflow/3_tasks.md`
...
- [ ] T009 Update `/4_implement` in `.gemini/commands/workflow/4_implement.md`
  - Add invocation for:
...
- [ ] T010 Update `/5_integrate` in `.gemini/commands/workflow/5_integrate.md`
  - Add invocation for:
...
- [ ] T011 Update `/6_release` in `.gemini/commands/workflow/6_release.md`
  - Add invocation for:
    1. `agentdb-state-manager/scripts/record_sync.py --pattern phase_6_release`
  - Keep existing release_workflow.py invocations
  - Update outputs to mention AgentDB state

- [ ] T012 Update `/7_backmerge` in `.gemini/commands/workflow/7_backmerge.md`
  - Add invocation for:
...
- [ ] T013 Update `/workflow/all` in `.gemini/commands/workflow/all.md`
...
- [ ] T018 Create `.specify/README.md` explaining deprecation
  - Note: "This directory is deprecated. See .gemini/skills/ for workflow automation."
...
- [ ] T019 [P] Move `sync_todo_to_db.py` to `ARCHIVED/` in workflow-utilities
  ```bash
  mv .gemini/skills/workflow-utilities/scripts/sync_todo_to_db.py \
     .gemini/skills/workflow-utilities/ARCHIVED/
  ```

- [ ] T020 [P] Move `todo_updater.py` to `ARCHIVED/` in workflow-utilities
  ```bash
  mv .gemini/skills/workflow-utilities/scripts/todo_updater.py \
     .gemini/skills/workflow-utilities/ARCHIVED/
  ```

- [ ] T021 [P] Move `workflow_archiver.py` to `ARCHIVED/` in workflow-utilities
  ```bash
  mv .gemini/skills/workflow-utilities/scripts/workflow_archiver.py \
     .gemini/skills/workflow-utilities/ARCHIVED/
  ```

- [ ] T022 [P] Move `workflow_registrar.py` to `ARCHIVED/` in workflow-utilities
  ```bash
  mv .gemini/skills/workflow-utilities/scripts/workflow_registrar.py \
     .gemini/skills/workflow-utilities/ARCHIVED/
  ```

- [ ] T023 [P] Move `sync_manifest.py` to `ARCHIVED/` in workflow-utilities
  ```bash
  mv .gemini/skills/workflow-utilities/scripts/sync_manifest.py \
     .gemini/skills/workflow-utilities/ARCHIVED/
  ```
...
- [ ] T024 Update `GEMINI.md` to reflect new workflow
...
- [ ] T025 Update `.gemini/skills/workflow-orchestrator/GEMINI.md`
  - Add references to new AgentDB scripts
  - Update skill loading logic description
  - Remove TODO file references

- [ ] T026 Update `.gemini/skills/agentdb-state-manager/GEMINI.md`
...
- [ ] T028 [P] Verify no .specify/ references in slash commands
  ```bash
  grep -r "\.specify/" .gemini/commands/workflow/ && echo "FAIL" || echo "PASS"
  ```
  - Expected: No matches

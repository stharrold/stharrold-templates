...
This feature unifies two systems (`.specify/` and `.claude/skills/`) into one. The data model focuses on:
...
| has_claude_md | bool | Has CLAUDE.md context file |
...
**Location**: `.claude-state/agentdb.duckdb` → `agent_synchronizations` table

| Field | Type | Description |
|-------|------|-------------|
| sync_id | UUID | Unique identifier |
| agent_id | string | `claude-code` |

...
| `/5_integrate` | `pr_workflow.py` | `.claude/skills/git-workflow-manager/` |
| `/6_release` | `release_workflow.py` | `.claude/skills/git-workflow-manager/` |
| `/7_backmerge` | `backmerge_workflow.py` | `.claude/skills/git-workflow-manager/` |
...
1. **Two disconnected systems exist**: `.specify/` (bash) and `.claude/skills/` (Python). The intended design uses only skills.
...
4. **Slash commands are prompts, not executables**: They provide instructions to Claude Code, which then calls the appropriate scripts.
...
- `workflow-orchestrator/CLAUDE.md` - Context detection algorithm
- `agentdb-state-manager/schemas/agentdb_sync_schema.sql` - State schema
- `git-workflow-manager/CLAUDE.md` - Script documentation

...
2. Extract key concepts from description
   - Actors: developers, Gemini Code, workflow orchestrator
...
### Primary User Story
As a developer, I want the slash commands (/1_specify through /7_backmerge) to invoke the appropriate skills (bmad-planner, speckit-author, git-workflow-manager, etc.) so that I have a unified, state-tracked workflow instead of two disconnected systems (.specify/ and .gemini/skills/).
...
### Key Entities
- **Slash commands**: `/1_specify` through `/7_backmerge` in `.gemini/commands/workflow/`
...
- **AgentDB**: DuckDB database for workflow state tracking in `.gemini-state/`

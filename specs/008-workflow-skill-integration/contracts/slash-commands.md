...
podman-compose run --rm dev python .claude/skills/tech-stack-adapter/scripts/detect_stack.py
...
**Invocations**:
```bash
# 1. Validate tasks exist (Claude reads plan.md directly)
...
**Invocations**:
```bash
# 1. Execute tasks (Claude uses TodoWrite internally)

...
**Location**: `.gemini/skills/agentdb-state-manager/scripts/record_sync.py`

**Purpose**: Record workflow transitions in AgentDB

**Interface**:
```bash
python .gemini/skills/agentdb-state-manager/scripts/record_sync.py \
  --sync-type <type> \
  --pattern <pattern> \
  [--source <source>] \
  [--target <target>] \
...
**Location**: `.gemini/skills/agentdb-state-manager/scripts/query_workflow_state.py`
...
CREATE TABLE IF NOT EXISTS agent_synchronizations (
    ...
    agent_id VARCHAR NOT NULL,          -- 'gemini-code'

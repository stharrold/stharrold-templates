...
**Breaking change: Python 3.11 -> 3.12 minimum.** Downstream repos
that must stay on Python 3.11 should pin to the 8.x series of this
template (`git clone --branch v8.7.0` or equivalent). The 9.x series
targets 3.12 for `datetime.UTC` (required by Ruff UP017 auto-fix),
improved error messages, and PEP 695 type parameter syntax used by
newer skill scripts.

Also collects all the CLAUDE.md hygiene work from early April 2026
as the inaugural 9.x release:
  - 17 autogenerated stub CLAUDE.md files deleted
  - `generate_claude_md.py` / `check_claude_md_frontmatter.py` /
    `migrate_directory_frontmatter.py` / `update_claude_md_refs.py`
    retired
  - `claude-md-frontmatter` pre-commit hook removed
  - 4 skill CLAUDE.md files rewritten (net -2315 lines, -85%)
  - Workflow slash commands renamed `v7x1_N` -> `sN` (step-N)
  - AgentDB split from `full` into opt-in `agentdb` bundle
  - New `claude-md-hygiene` skill with content-density auditor
  - New `scripts/pre_release_sanity.py` local CI gate
...
1. **Verify Python 3.12 is available**: `python3.12 --version`.
   On macOS: `brew install python@3.12`. On Ubuntu: use deadsnakes
   PPA or build from source. CI runners on GitHub Actions have 3.12
   pre-installed.
2. **Re-run `uv sync`** to regenerate `uv.lock` against 3.12.
3. **Update your own repo's `pyproject.toml`** if it inherits from
   the template's conventions (`target-version`, `python_version`,
   `requires-python`).
...
- **`agentdb` bundle (new)** -- Opt-in AgentDB state tracking, previously bundled with `full`. See BUNDLES.md "Lessons incorporated" for rationale. Breaking change for users of `--bundle full` who depended on AgentDB being installed implicitly.
- **"Known CI gotchas" section in BUNDLES.md** under the `ci` bundle -- documents the `lfs: true`, `concurrency:`, `npm audit --omit=dev`, multi-project Playwright `--project=<name>`, and `claude-code-review.yml` configuration patterns that came from running these workflows in downstream consumer repos.
- **"Lessons incorporated" section in BUNDLES.md** -- provenance trail linking each convention back to the concrete synavistra PR/commit that motivated it.
- **`what goes here` rubric in root CLAUDE.md** -- explicit 30-minute threshold, remove-when-stale priority, cross-refs to BUNDLES.md/WORKFLOW.md instead of duplicating architecture in CLAUDE.md.

### Changed
- **`tests.yml` shipped by the `ci` bundle now sets `lfs: true` on every `actions/checkout@v4` step** and includes a `concurrency:` block keyed by workflow + PR number. Downstream consumers pick these up automatically on the next `--bundle ci` re-apply. Traced to synavistra PR #706 where the source-registry validator had been silently failing on LFS pointer files for 3+ weeks.
- **`claude-code-review.yml` and `secrets-example.yml` also set `lfs: true` on checkout.**
- **Slash command prefix renamed from `v7x1_N` to `sN`** (step-N). Files under `.claude/commands/workflow/`:
  - `v7x1_1-worktree.md` -> `s1-worktree.md`
  - `v7x1_2-integrate.md` -> `s2-integrate.md`
  - `v7x1_3-release.md` -> `s3-release.md`
  - `v7x1_4-backmerge.md` -> `s4-backmerge.md`
  - In-file references updated in 27 active files (209 edits). Historical files (CHANGELOG, ARCHIVED/, docs/archived/) intentionally left unchanged.
- **Internal AgentDB schema keys `phase_v7x1_1_worktree` .. `phase_v7x1_4_backmerge` are deliberately NOT renamed** -- they are stable protocol between `record_sync.py` and `query_workflow_state.py`, and renaming would require a data migration for existing AgentDB databases. A backward-compat comment was added at each declaration site.
- **Workflow skill CLAUDE.md files pruned** (git-workflow-manager, workflow-orchestrator, workflow-utilities, agentdb-state-manager, initialize-repository, tech-stack-adapter): removed stale `type: claude-context` frontmatter, removed auto-generated "Related Documentation" + "Child Directories" footer blocks, removed references to archived skills (bmad-planner, speckit-author, quality-enforcer). Algorithm pseudocode in workflow-orchestrator CLAUDE.md flagged as scheduled-for-rewrite but left unchanged to avoid scope creep.

### Removed
- **Autogenerated per-directory CLAUDE.md stubs** (22 files, ~700 lines):
  - `tests/{,contract,integration,unit}/CLAUDE.md`
  - `docs/{,archived,reference,research,guides/40_deployment}/CLAUDE.md`
  - `.claude/{,commands,commands/workflow,skills}/CLAUDE.md`
  - `.claude/skills/*/scripts/CLAUDE.md` (6 files)
  - `.claude/skills/*/templates/CLAUDE.md` (2 files)
  - `.claude/skills/agentdb-state-manager/schemas/CLAUDE.md`
  Downstream experience (synavistra) showed these were actively harmful: every file was an `ls` equivalent with frontmatter, consuming context tokens in every session that navigated near them with zero information payoff.
- **CLAUDE.md stub generator subsystem** (4 files, ~500 lines):
  - `workflow-utilities/scripts/generate_claude_md.py` -- auto-scanned a hardcoded `TARGET_DIRS` list and stamped stub files into any directory without one
  - `workflow-utilities/scripts/check_claude_md_frontmatter.py` -- the pre-commit hook script that enforced `type: claude-context` on every CLAUDE.md
  - `workflow-utilities/scripts/migrate_directory_frontmatter.py` -- migration helper for the frontmatter format
  - `workflow-utilities/scripts/update_claude_md_refs.py` -- children-reference updater
- **`claude-md-frontmatter` pre-commit hook** -- the rule that required every CLAUDE.md to have YAML frontmatter, which is what kept the stub pattern alive.
- **`agentdb-state-manager` from the `full` bundle** (BREAKING). See "Added" above for the new opt-in `agentdb` bundle. Users of `--bundle full` who rely on AgentDB must now also run `--bundle agentdb`.

### Fixed
- **Two pre-existing em-dash violations** (`scripts/run_entity_quality.py`, `data_catalog/services/grain_discovery.py`) that the `ascii-only` pre-commit hook was flagging. Drive-by fix during the CLAUDE.md cleanup.

### Migration notes for downstream consumers
1. Re-apply the `ci` bundle to pick up the `lfs: true` and concurrency fixes:
   ```bash
   python .tmp/stharrold-templates/scripts/apply_bundle.py .tmp/stharrold-templates . --bundle ci
   ```
2. Update any references to `v7x1_N` slash commands in your own CLAUDE.md / docs / scripts to use `sN`.
3. If your pipeline depends on AgentDB tracking, add `--bundle agentdb` to your next apply. `full` no longer includes it.
4. If you use the `git` or `full` bundle, delete any Tier-B stub CLAUDE.md files your tree inherited from earlier template versions. They are no longer generated. See synavistra commit `15a8f2b` for the exact list.

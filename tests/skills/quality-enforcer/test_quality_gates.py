...
sys.path.insert(
    0,
    str(Path(__file__).parent.parent.parent.parent / ".claude/skills/quality-enforcer/scripts"),
)
...
class TestCheckCoverage:
    ...
    @patch("run_quality_gates.subprocess.run")
    def test_check_coverage_calls_script(self, mock_run):
...
class TestCheckBuild:
    ...
    @patch("run_quality_gates.subprocess.run")
    def test_check_build_success(self, mock_run):
    ...
    @patch("run_quality_gates.subprocess.run")
    def test_check_build_failure(self, mock_run):
...
class TestRunAllQualityGates:
    ...
    def test_all_gates_pass(self, mock_worktree, mock_coverage, mock_tests, mock_build, mock_lint):
    ...
    @patch("run_quality_gates.check_coverage")
    @patch("run_quality_gates.get_worktree_info")
    def test_fails_when_one_gate_fails(self, mock_worktree, mock_coverage, mock_tests, mock_build, mock_lint):
    ...
    @patch("run_quality_gates.check_coverage")
    @patch("run_quality_gates.get_worktree_info")
    def test_coverage_threshold_passed_to_check_coverage(self, mock_worktree, mock_coverage, mock_tests, mock_build, mock_lint):

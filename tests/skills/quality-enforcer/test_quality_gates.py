...
class TestSyncAiConfig:
    ...
    def test_sync_skipped_when_no_changes(self, mock_run):
        """Should skip sync when CLAUDE.md not modified."""
    ...
    @patch("run_quality_gates.subprocess.run")
    def test_sync_triggered_when_claude_md_modified(self, mock_run, mock_path, mock_copy):
        """Should perform sync when CLAUDE.md is modified."""
        mock_run.return_value = MagicMock(returncode=0, stdout="CLAUDE.md")

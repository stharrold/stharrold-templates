...
class TestContainerDetection:
    ...
    def test_docker_detected_via_dockerenv(self):
        ...
        with patch.object(Path, "exists", return_value=True):
            ...
            assert run.is_container() is True
    ...
    def test_no_container_when_no_indicators(self):
        ...
        with patch.object(Path, "exists", return_value=False):
            with patch.object(Path, "read_text", return_value="1:name=systemd:/"):
                assert run.is_container() is False
...
class TestSecretResolution:
    ...
    def test_env_var_takes_precedence(self):
        ...
        with patch.dict(os.environ, {"TEST_SECRET": "from_env"}, clear=False):
            value, source = run.resolve_secret("TEST_SECRET", "test-service", False, False)
    ...
    def test_ci_mode_requires_env_var(self):
        ...
        with patch.dict(os.environ, {}, clear=False):
            ...
            value, source = run.resolve_secret("TEST_SECRET", "test-service", True, False)
    ...
    def test_container_mode_requires_env_var(self):
        ...
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("TEST_SECRET", None)
            value, source = run.resolve_secret("TEST_SECRET", "test-service", False, True)
            assert value is None
            assert "CI/container mode" in source
    ...
    def test_local_mode_uses_keyring(self):
        ...
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("TEST_SECRET", None)
            with patch.object(run, "get_secret_from_keyring", return_value="from_keyring"):
                value, source = run.resolve_secret("TEST_SECRET", "test-service", False, False)
...
"""
        ...
        with patch.object(run, "get_repo_root", return_value=tmp_path):
            with patch.object(run, "is_ci", return_value=False):
                with patch.object(run, "is_container", return_value=False):
                    with patch.object(run, "get_secret_from_keyring", return_value="test_value"):

...
- `claude-code-review.yml` requires `allowed_tools: "Bash,WebFetch,WebSearch,Skill,Task"` so Claude's tool calls aren't denied in CI, and `fetch-depth: 0` so `git diff` can reach the base branch

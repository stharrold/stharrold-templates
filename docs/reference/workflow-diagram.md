...
Visual representation of the streamlined sN workflow system.
...
flowchart TB
    subgraph Feature["Feature Workflow"]
        Step1[/workflow:s1-worktree<br/>Create isolated<br/>environment]
        Step2[Implementation<br/>Autonomous development<br/>using Gemini tools]
        Step3[/workflow:s2-integrate<br/>Create PRs from<br/>feature to develop]

        Step1 --> Step2 --> Step3
    end

    subgraph Release["Release Workflow"]
        Step4[/workflow:s3-release<br/>develop → release → main]
        Step5[/workflow:s4-backmerge<br/>Sync release to<br/>develop and contrib]
...
flowchart LR
    ...
    Step1[/workflow:s1-worktree] --> Loop
    Loop -->|Done| Step3[/workflow:s2-integrate]
```

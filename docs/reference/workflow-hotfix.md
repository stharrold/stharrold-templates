...
**Branch:** `main`
**Command:** `/workflow:s1-worktree "hotfix: brief description"`

The `/workflow:s1-worktree` command handles hotfix creation when run from the `main` branch (or by specifying `main` as base).
...
**Location:** Main repository
**Command:** `/workflow:s2-integrate "hotfix/branch-name"`
...
2. **Tag Release**: After merging, use `/workflow:s3-release` to tag the new production version (e.g., `v7.0.1`).
3. **Backmerge to Develop**: Use `/workflow:s4-backmerge` to sync the fix into the `develop` branch.

...
**Branch:** `main`
**Command:** `/worktree "hotfix: brief description"`

The `/worktree` command handles hotfix creation when run from the `main` branch (or by specifying `main` as base).
...
**Location:** Main repository
**Command:** `/integrate "hotfix/branch-name"`
...
2. **Tag Release**: After merging, use `/release` to tag the new production version (e.g., `v7.0.1`).
3. **Backmerge to Develop**: Use `/backmerge` to sync the fix into the `develop` branch.

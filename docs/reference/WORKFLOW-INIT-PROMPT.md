...
User downloads german release to untracked directory in their repo, then tells Gemini Code:
...
→ **Read:** `.gemini/skills/initialize-repository/SKILL.md` (~500 tokens)
...
→ **Read:** `.gemini/skills/bmad-planner/SKILL.md` (~600 tokens)
→ **Purpose:** Interactive planning tool (Business Model, Architecture, Design)
→ **Alternative:** `.gemini/skills/speckit-author/SKILL.md` if skipping BMAD planning

### Implementing a Feature (Phase 2-3)?
→ **Read:** `.gemini/skills/git-workflow-manager/SKILL.md` (~700 tokens)
→ **Purpose:** Worktree creation, branch management, semantic versioning
→ **Then:** `.gemini/skills/speckit-author/SKILL.md` (~800 tokens)
→ **Purpose:** Create specifications and implementation plans
→ **Then:** `.gemini/skills/quality-enforcer/SKILL.md` (~400 tokens)
...
### 1. `.gemini/skills/initialize-repository/SKILL.md` (~500 tokens)
...
### 2. `GEMINI.md` - Sections: "Workflow v5.2 Architecture" + "Available Skills" (~300 tokens)
...
**Essential:**
- `.gemini/skills/initialize-repository/SKILL.md` - Bootstrap new repos
- `.github/workflows/tests.yml` - GitHub Actions CI/CD reference
- `azure-pipelines.yml` - Azure Pipelines CI/CD reference

**Commands:**
```bash
python .gemini/skills/initialize-repository/scripts/initialize_repository.py <source> <target>
```
...
**Essential:**
- `.gemini/skills/bmad-planner/SKILL.md` - Interactive planning with 3 personas
- `WORKFLOW.md` - Phase 1 section (lines ~200-400)

**Commands:**
```bash
python .gemini/skills/bmad-planner/scripts/create_planning.py <slug> <gh_user>
```
...
**Essential:**
- `.gemini/skills/git-workflow-manager/SKILL.md` - Worktree and branch management
- `.gemini/skills/speckit-author/SKILL.md` - Specification generation
- `.gemini/skills/quality-enforcer/SKILL.md` - Quality gates
- `WORKFLOW.md` - Phase 2-3 sections (lines ~400-800)

**Commands:**
```bash
# Create worktree
python .gemini/skills/git-workflow-manager/scripts/create_worktree.py feature <slug> contrib/<gh_user>

# Create specifications (in worktree)
cd ../german_feature_<slug>
python .gemini/skills/speckit-author/scripts/create_specifications.py feature <slug> <gh_user> --todo-file ../TODO_feature_*.md

# Run quality gates
python .gemini/skills/quality-enforcer/scripts/run_quality_gates.py

# Calculate semantic version
python .gemini/skills/git-workflow-manager/scripts/semantic_version.py develop v1.5.0
```
...
**Essential:**
- `.gemini/skills/workflow-utilities/SKILL.md` - TODO management and archival
- `.gemini/skills/speckit-author/SKILL.md` - As-built documentation update
- `WORKFLOW.md` - Phase 4 section (lines ~800-1000)

**Commands:**
```bash
# After PR merge: Archive workflow
python .gemini/skills/workflow-utilities/scripts/workflow_archiver.py TODO_feature_*.md --summary "Completed feature" --version "1.6.0"

# Update BMAD with as-built (optional)
python .gemini/skills/speckit-author/scripts/update_asbuilt.py planning/<slug> specs/<slug>

# Daily rebase contrib
python .gemini/skills/git-workflow-manager/scripts/daily_rebase.py contrib/<gh_user>
```
...
**Essential:**
- `.gemini/skills/git-workflow-manager/SKILL.md` - Release scripts
- `WORKFLOW.md` - Phase 5 section (lines ~1000-1400)

**Commands:**
```bash
# Create release branch
python .gemini/skills/git-workflow-manager/scripts/create_release.py v1.6.0 develop

# After PR merge: Tag release
python .gemini/skills/git-workflow-manager/scripts/tag_release.py v1.6.0 main

# Back-merge to develop
python .gemini/skills/git-workflow-manager/scripts/backmerge_release.py v1.6.0 develop

# Cleanup release branch
python .gemini/skills/git-workflow-manager/scripts/cleanup_release.py v1.6.0
```
...
**Essential:**
- `.gemini/skills/git-workflow-manager/SKILL.md` - Hotfix worktree creation
- `WORKFLOW.md` - Phase 6 section (lines ~1400-1600)

**Commands:**
```bash
# Create hotfix worktree
python .gemini/skills/git-workflow-manager/scripts/create_worktree.py hotfix <slug> main

# (Optional) Create specifications for complex fixes
python .gemini/skills/speckit-author/scripts/create_specifications.py hotfix <slug> <gh_user>

# Run quality gates
python .gemini/skills/quality-enforcer/scripts/run_quality_gates.py

# After PR merge: Tag hotfix
python .gemini/skills/git-workflow-manager/scripts/tag_release.py v1.3.0-hotfix.1 main

# Back-merge to develop
python .gemini/skills/git-workflow-manager/scripts/backmerge_release.py v1.3.0-hotfix.1 develop
```
...
- `.gemini/skills/UPDATE_CHECKLIST.md` - How to update skills properly
...
See `.gemini/skills/workflow-utilities/SKILL.md` for directory creation utilities.
...
Enforced by: `.gemini/skills/quality-enforcer/scripts/run_quality_gates.py`
...
**At 100K tokens:**
1. Gemini automatically saves state to TODO_*.md
...
See `.gemini/skills/workflow-orchestrator/SKILL.md` for details.

---

## Official Gemini Code Documentation

This workflow extends official Gemini Code patterns:

- **Skills:** https://docs.gemini.com/en/docs/agents-and-tools/agent-skills
- **Building Agents:** https://docs.gemini.com/en/docs/agents-and-tools/building-agents
- **Getting Started:** https://docs.gemini.com/en/docs/gemini-code/getting-started
- **Docs Map:** https://docs.gemini.com/en/docs/gemini-code/gemini_code_docs_map.md

**Skill creation:** Use `.gemini/skills/workflow-utilities/scripts/create_skill.py <skill-name>` - automatically fetches official docs and compares patterns.

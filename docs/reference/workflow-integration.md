...
This document covers the synchronization and deployment steps of the sN workflow.
...
If you provide the branch name, `/workflow:s2-integrate` will:
...
**Command:** `/workflow:s3-release [version]`
...
The `/workflow:s3-release` command uses semantic versioning (`vMAJOR.MINOR.PATCH`):

...
gemini oauth refresh --provider github --force-rotation
gemini oauth validate --all-providers --health-check

# Enterprise SSO integration
gemini oauth configure-sso --provider okta --domain company.okta.com
```
...
spec:
  ...
  - from:
    - source:
        principals: ["cluster.local/ns/gemini-code/sa/gemini-client"]
  - to:
    - operation:
        methods: ["POST", "GET"]
        paths: ["/mcp/*"]

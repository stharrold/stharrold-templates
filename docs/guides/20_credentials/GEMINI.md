...
changelog:
  ...
  - Integrated GEMINI.md project context template (v1.0)
...
→ **Complete credential setup first** before proceeding to [../10_mcp/GEMINI.md](../10_mcp/GEMINI.md)

## Related Concepts

- **MCP server configuration**: [../10_mcp/GEMINI.md](../10_mcp/GEMINI.md)
- **Implementation strategies**: [../30_implementation/GEMINI.md](../30_implementation/GEMINI.md)
...
- The `${env:VARIABLE_NAME}` syntax in gemini.json reads from environment variables
...
1. Configure MCP servers using [../10_mcp/GEMINI.md](../10_mcp/GEMINI.md)
2. Use `mcp_manager.py --check-credentials` to validate setup
3. Test servers with `/mcp` command in Gemini Code
4. Review [../30_implementation/GEMINI.md](../30_implementation/GEMINI.md) for phased rollout strategy

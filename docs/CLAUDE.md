---
type: claude-context
directory: docs
purpose: Documentation root for guides, references, and archives.
parent: ../CLAUDE.md
sibling_readme: null
children:
  - archived/CLAUDE.md
  - guides/CLAUDE.md
  - reference/CLAUDE.md
  - research/CLAUDE.md
---

# Claude Code Context: docs

## Purpose

Documentation root for guides, references, and archives.

## Contents

- `archived/` - Subdirectory
- `guides/` - Subdirectory
- `reference/` - Subdirectory
- `research/` - Subdirectory

## Related

- **Parent**: [stharrold-templates](../CLAUDE.md)
- **archived**: [archived/CLAUDE.md](archived/CLAUDE.md)
- **guides**: [guides/CLAUDE.md](guides/CLAUDE.md)
- **reference**: [reference/CLAUDE.md](reference/CLAUDE.md)
- **research**: [research/CLAUDE.md](research/CLAUDE.md)

## Related Documentation

- **[../CLAUDE.md](../CLAUDE.md)** - Parent directory: Root

**Child Directories:**
- **[archived/CLAUDE.md](archived/CLAUDE.md)** - Archived
- **[guides/CLAUDE.md](guides/CLAUDE.md)** - Guides
- **[reference/CLAUDE.md](reference/CLAUDE.md)** - Reference
- **[research/CLAUDE.md](research/CLAUDE.md)** - Research

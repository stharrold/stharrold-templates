...
def create_directory_structure(planning_dir: Path, slug: str) -> None:
    """Create compliant planning directory structure with GEMINI.md, README.md, ARCHIVED/."""

    planning_dir.mkdir(parents=True, exist_ok=True)

    # Create GEMINI.md
    gemini_md = planning_dir / "GEMINI.md"
    if not gemini_md.exists():
        gemini_content = f"""# Gemini Code Context: {slug} Planning
...
"""
    ...
    if not archived_gemini.exists():
        archived_gemini.write_text(
            f"""# Gemini Code Context: Archived Planning
...
"""
        ...
        print(f"  [OK] Created {archived_gemini}")
...
def main():
    ...
    print("  - GEMINI.md (Context for Gemini Code)")
    ...
    print("  2. Create feature worktree: python .gemini/skills/git-workflow-manager/scripts/create_worktree.py feature {args.slug} {context['current_branch']}")

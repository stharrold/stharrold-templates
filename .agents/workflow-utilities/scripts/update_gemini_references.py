...
def generate_cross_references(gemini_file: Path, root_dir: Path) -> str:
    ...
    if dir_path != root_dir:
        ...
        if parent_gemini.exists():
            ...
            refs.append(f"- **[../{parent_gemini.name}]({rel_path})** - Parent directory: {parent_name}")
    ...
    for child_dir in child_dirs:
        ...
        if child_gemini.exists():
            ...
            child_geminis.append(f"- **[{child_dir.name}/GEMINI.md]({rel_path})** - {child_dir.name.replace('-', ' ').replace('_', ' ').title()}")
...
def update_gemini_file(gemini_file: Path, root_dir: Path, dry_run: bool = False):
    ...
    if dry_run:
        ...
        print(f"✓ Updated {gemini_file.relative_to(root_dir)}")
...
def main():
    ...
    print()
    print(f"✅ {'Would update' if dry_run else 'Updated'} {len(gemini_files)} GEMINI.md files")

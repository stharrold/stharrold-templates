...
def update_skill_md_version(skill_md: Path, old_version: str, new_version: str, dry_run: bool = False) -> bool:
    ...
    new_content = re.sub(r"(version:\s*)" + re.escape(old_version), r"\g<1>" + new_version, content, count=1)
...
- .gemini/skills/{skill_name}/SKILL.md (version {new_version})
- .gemini/skills/{skill_name}/CHANGELOG.md (new entry)

Refs: .gemini/skills/{skill_name}/CHANGELOG.md
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Semi-automated documentation sync tool for skill updates")
    ...
    # Paths
    skill_dir = repo_root / ".gemini" / "skills" / args.skill_name
    ...
    print("  3. Update root GEMINI.md if command examples changed")
    print("  4. Run: python .gemini/skills/workflow-utilities/scripts/validate_versions.py")
    print("  5. Review complete UPDATE_CHECKLIST: cat .gemini/skills/UPDATE_CHECKLIST.md")

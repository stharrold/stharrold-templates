...
def sync_gemini_md_to_agents() -> tuple[bool, bool]:
    """
    Sync GEMINI.md to AGENTS.md.
    ...
    if not source.exists():
        safe_print("  " + format_warning("GEMINI.md not found, skipping AGENTS.md sync"))
        return True, False
    ...
    try:
        ...
        safe_print("  " + format_arrow("GEMINI.md", "AGENTS.md"))
        return True, True
...
def sync_gemini_md_to_copilot() -> tuple[bool, bool]:
    """
    Sync GEMINI.md to .github/copilot-instructions.md.
    ...
    if not source.exists():
        safe_print("  " + format_warning("GEMINI.md not found, skipping copilot-instructions.md sync"))
        return True, False
    ...
    try:
        shutil.copy(source, dest)
        safe_print("  " + format_arrow("GEMINI.md", ".github/copilot-instructions.md"))
        return True, True
    except Exception as e:
...
def sync_skills_to_agents_dir() -> tuple[bool, bool]:
    """
    Sync .gemini/skills/ to .agents/.

    Returns:
        (success: bool, modified: bool)
    """
    source_dir = Path(".gemini/skills")
    dest_dir = Path(".agents")

    if not source_dir.exists():
        safe_print("  " + format_warning(".gemini/skills/ not found, skipping .agents/ sync"))
        return True, False
    ...
    try:
        ...
        if modified:
            safe_print("  " + format_arrow(".gemini/skills/", ".agents/"))
...
def sync_all() -> tuple[bool, bool]:
    ...
    # Sync GEMINI.md → AGENTS.md
    success, modified = sync_gemini_md_to_agents()
    all_success &= success
    any_modified |= modified

    # Sync GEMINI.md → .github/copilot-instructions.md
    success, modified = sync_gemini_md_to_copilot()
    all_success &= success
    any_modified |= modified

    # Sync .gemini/skills/ → .agents/
...
def verify_sync() -> bool:
    ...
    if source.exists():
        if not dest.exists():
            ...
            safe_print("  " + format_cross("AGENTS.md differs from GEMINI.md"))
            all_synced = False
        else:
            safe_print("  " + format_check("AGENTS.md in sync"))

    # Verify GEMINI.md → .github/copilot-instructions.md
    dest = Path(".github/copilot-instructions.md")
    if source.exists():
        if not dest.exists():
            ...
            safe_print("  " + format_cross(".github/copilot-instructions.md differs from GEMINI.md"))
            all_synced = False
        else:
            safe_print("  " + format_check(".github/copilot-instructions.md in sync"))

    # Verify .gemini/skills/ → .agents/
    source_dir = Path(".gemini/skills")
    dest_dir = Path(".agents")
                ...
                if skill_dir.is_dir():
                    ...
                    if not dest.exists():
                        ...
                        if _dirs_differ(comparison):
                            safe_print("  " + format_cross(f".agents/{skill_dir.name}/ differs from .gemini/skills/{skill_dir.name}/"))
                            skills_synced = False
            if skills_synced:
                safe_print("  " + format_check(".agents/ in sync with .gemini/skills/"))
            else:
                all_synced = False

...
3. copy-gemini-to-agents - Sync GEMINI.md to AGENTS.md and .agents/
4. pr-start-contrib-to-develop - Create PR from contrib to develop

Usage:
    podman-compose run --rm dev python .gemini/skills/git-workflow-manager/scripts/pr_workflow.py <step>

Steps:
    finish-feature    - Step 1: PR feature -> contrib (runs quality gates first)
    archive-todo      - Step 2: Archive TODO file after merge
    sync-agents       - Step 3: Sync GEMINI.md -> AGENTS.md
...
def step_sync_agents() -> bool:
    """
    Step 3: Sync GEMINI.md to AGENTS.md and .agents/.
    ...
    """
    print("\n" + "=" * 60)
    print("STEP 3: Sync GEMINI.md -> AGENTS.md")
    print("=" * 60)
    ...
    try:
        ...
        if Path("GEMINI.md").exists():
            shutil.copy("GEMINI.md", "AGENTS.md")
            safe_print("  " + format_check(" GEMINI.md -> AGENTS.md"))
            Path(".github").mkdir(exist_ok=True)
            shutil.copy("GEMINI.md", ".github/copilot-instructions.md")
            safe_print("  " + format_check(" GEMINI.md -> .github/copilot-instructions.md"))
        if Path(".gemini/skills").exists():
            Path(".agents").mkdir(exist_ok=True)
            for skill_dir in Path(".gemini/skills").iterdir():
                if skill_dir.is_dir():
                    dest = Path(".agents") / skill_dir.name
                    if dest.exists():
                        shutil.rmtree(dest)
                    shutil.copytree(skill_dir, dest)
            safe_print("  " + format_check(" .gemini/skills/ -> .agents/"))
        modified = True
    ...
    if modified:
        ...
        result = run_cmd(
            [
                ...
                "commit",
                "-m",
                "chore: sync GEMINI.md to cross-tool formats\n\n"
                "🤖 Generated with [Gemini Code](https://gemini.com/gemini-code)\n\n"
                "Co-Authored-By: Gemini <noreply@anthropic.com>",
            ],
            check=False,

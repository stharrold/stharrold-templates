...
def generate_spec_md(
    ...
    # (This helps Gemini Code understand implementation preferences)
...
def generate_plan_md(slug: str, workflow_type: str, date: str, qa_responses: dict[str, any], bmad_docs: dict[str, Path] | None) -> str:
    ...
    note += "<!-- This template provides the structure. Gemini Code will populate with actual tasks. -->\n\n"
...
def create_specs_directory(slug: str, workflow_type: str, planning_path: str | None = None) -> Path:
    ...
    # Create GEMINI.md
    gemini_md = f"""# Gemini Code Context: specs/{slug}
...
- **[{planning_ref}/GEMINI.md]({planning_ref}/GEMINI.md)** - BMAD Planning (if available)
"""

    (specs_dir / "GEMINI.md").write_text(gemini_md)
...
"""
    ...
    (archived_dir / "GEMINI.md").write_text(f"# Gemini Code Context: specs/{slug}/ARCHIVED\n\nArchived specifications and deprecated files.\n")
...
def main():
    ...
    if not tasks:
        ...
        print("  Gemini Code will populate plan.md with actual tasks")
    else:

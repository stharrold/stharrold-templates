...
class RepositoryConfig:
    ...
    def __init__(self):
        ...
        self.vcs_provider: str = "github"
        self.vcs_user: str = ""  # GitHub username
...
def phase1_configuration(source_path: Path, target_path: Path) -> RepositoryConfig:
    ...
    config.copy_cicd = ask_yes_no("Copy CI/CD pipelines (.github/workflows/)?", default=True)
...
def generate_readme(config: RepositoryConfig, target_path: Path) -> None:
    ...
    vcs_prereq = "- Git and GitHub CLI (`gh`)"
...
def print_summary(target_path: Path, config: RepositoryConfig) -> None:
    ...
    print(f"{Colors.BLUE}VCS Provider:{Colors.END} GitHub")
    print(f"{Colors.BLUE}GitHub User:{Colors.END} {config.vcs_user}")
    ...
    if config.copy_cicd:
        print("  [OK] CI/CD pipelines (GitHub Actions)")
...
""",
    ...
    print("  VCS Provider: GitHub")
    print(f"  GitHub User: {config.vcs_user}")

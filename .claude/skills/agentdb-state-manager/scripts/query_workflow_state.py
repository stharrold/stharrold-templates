            ...
            records.append(
                {
                    ...
                    "worktree_id": "main" if result[1] is None else Path(result[1]).name if result[1] else "main",
                    "sync_type": result[2],
                    "pattern": result[3],
                    "status": result[4],
...
def query_latest_sync(db_path: Path, worktree: str | None = None) -> dict | None:
    ...
    try:
        ...
        # Fallback to CLI
        result = subprocess.run(["duckdb", str(db_path), "-json", "-c", sql], capture_output=True, text=True)
...
""",
    ...
    parser.add_argument("--all", action="store_true", help="Show records from all sessions/worktrees (requires shared AgentDB)")
    parser.add_argument("--limit", type=int, default=20, help="Limit records when using --all (default: 20)")

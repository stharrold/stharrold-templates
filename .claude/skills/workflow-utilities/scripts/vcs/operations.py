...
def get_username(*, fallback: str | None = "", provider: VCSProvider | None = None) -> str:
    ...
    try:
        if provider == VCSProvider.GITHUB:
            ...
            return _run([AZURE_CLI, "devops", "user", "show", "--query", "user.uniqueName", "-o", "tsv"], timeout=10)
    except RuntimeError:
        if fallback is None:
            raise
        return fallback
...
def get_contrib_branch(*, fallback: str | None = "stharrold", provider: VCSProvider | None = None) -> str:

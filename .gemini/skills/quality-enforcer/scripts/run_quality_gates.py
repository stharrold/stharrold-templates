...
def check_coverage(threshold=80):
    ...
    script_path = ".gemini/skills/quality-enforcer/scripts/check_coverage.py"
    prefix = get_command_prefix()
...
def sync_ai_config():
    ...
    try:
        ...
        # Check if GEMINI.md or .gemini/ was modified
        ...
        needs_sync = "GEMINI.md" in modified_files or ".gemini/" in modified_files

        if not needs_sync:
            print("[WARN] GEMINI.md not modified, skipping sync")
            return True

        print("[INFO] GEMINI.md modified - syncing to cross-tool formats...")

        if Path("GEMINI.md").exists():
            shutil.copy("GEMINI.md", "AGENTS.md")
            print("  [OK] Synced GEMINI.md -> AGENTS.md")
            Path(".github").mkdir(exist_ok=True)
            shutil.copy("GEMINI.md", ".github/copilot-instructions.md")
            print("  [OK] Synced GEMINI.md -> .github/copilot-instructions.md")
        ...
        if Path(".gemini/skills").exists():
            ...
            print("  [OK] Synced .gemini/skills/ -> .agents/")

...
def check_coverage(threshold=80):
    ...
    try:
        ...
        # Measure coverage for: .gemini/skills, tools, and root Python files
        subprocess.run(
            ["uv", "run", "pytest", "--cov=.gemini/skills", "--cov=tools", "--cov=mcp_manager.py", "--cov-report=term", "--cov-report=json"],
            capture_output=True,
            text=True,
            check=False,
        )

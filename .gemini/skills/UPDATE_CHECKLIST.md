...
**File:** `.gemini/skills/<skill-name>/SKILL.md`
...
### Step 3: Update the Skill's GEMINI.md File

**File:** `.gemini/skills/<skill-name>/GEMINI.md`
...
**Files:** `.gemini/skills/<skill-name>/templates/*.template`
...
- [ ] **Other dependent skills**:
  ...
  - [ ] Update their GEMINI.md references

### Step 8: Create CHANGELOG Entry

**File:** `.gemini/skills/<skill-name>/CHANGELOG.md` (create if doesn't exist)
...
- [ ] Command examples are identical across SKILL.md, GEMINI.md, WORKFLOW.md
...
**NEW:** Check if changes align with official Gemini Code best practices:

```bash
# Review official docs for your skill type
# https://docs.gemini.com/en/docs/agents-and-tools/agent-skills
# https://docs.gemini.com/en/docs/agents-and-tools/building-agents
```
...
- [ ] **Document discrepancies:** If diverging, document rationale in SKILL.md:
  ...
  **Citation:** https://docs.gemini.com/en/docs/agents-and-tools/agent-skills
...
- Local: `SKILL.md`, `GEMINI.md`, `README.md`, `CHANGELOG.md`, `ARCHIVED/`
- Citation: https://docs.gemini.com/en/docs/agents-and-tools/agent-skills
- Rationale: Extended structure supports multi-phase workflow with version
  tracking (CHANGELOG.md), Gemini Code context (GEMINI.md), and archival
  system (ARCHIVED/).
```
...
```bash
git add .gemini/skills/<skill-name>/ WORKFLOW.md GEMINI.md
...
- GEMINI.md (version history, commands)
...
Refs: .gemini/skills/<skill-name>/CHANGELOG.md
...
- [ ] Run the updated skill script manually:
  ```bash
  python .gemini/skills/<skill-name>/scripts/<script-name>.py --help
  ```
...
cp .gemini/skills/<skill-name>/SKILL.md \
   .gemini/skills/<skill-name>/ARCHIVED/SKILL_vX.Y.Z.md
...
├── GEMINI.md         ← Update: usage, examples
...
GEMINI.md             ← Update: commands, version history
```

### Integration Files (Check if Applicable)
```
.gemini/skills/workflow-orchestrator/SKILL.md    ← If orchestrator calls updated
.gemini/skills/bmad-planner/SKILL.md             ← If BMAD integrates
.gemini/skills/speckit-author/SKILL.md           ← If SpecKit integrates
.gemini/skills/git-workflow-manager/SKILL.md     ← If git operations changed
.gemini/skills/quality-enforcer/SKILL.md         ← If quality gates changed
.gemini/skills/workflow-utilities/SKILL.md       ← If utilities changed
```
...
- Command syntax must match exactly in SKILL.md, GEMINI.md, WORKFLOW.md
...
5. ✓ `.gemini/skills/bmad-planner/CHANGELOG.md`
   - Added [5.1.0] entry with new feature

6. ✓ `.gemini/skills/speckit-author/SKILL.md`
...
Updated documentation:
- .gemini/skills/bmad-planner/SKILL.md (version 5.1.0, Q&A flow)
- .gemini/skills/bmad-planner/GEMINI.md (example updated)
- WORKFLOW.md (Phase 1 interactive session)
- GEMINI.md (Phase 1 description)
- .gemini/skills/speckit-author/SKILL.md (integration note)

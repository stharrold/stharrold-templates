...
# Gemini Code Context: initialize-repository
...
```
.gemini/skills/initialize-repository/
├── scripts/
...
1. **Validation (Pre-flight checks):**
   ...
   - Checks source has .gemini/skills/ directory
...
4. **Phase 3: File Operations (Automatic - no user input):**
   ...
   **Always copied verbatim:**
   - `.gemini/skills/workflow-orchestrator/` (main coordinator)
   - `.gemini/skills/tech-stack-adapter/` (project detection)
   - `.gemini/skills/git-workflow-manager/` (git automation)
   - `.gemini/skills/bmad-planner/` (planning Q&A)
   - `.gemini/skills/speckit-author/` (specification generation)
   - `.gemini/skills/quality-enforcer/` (quality gates)
   - `.gemini/skills/workflow-utilities/` (shared utilities)
   - `.gemini/skills/initialize-repository/` (this meta-skill)
   - `WORKFLOW.md` (complete 6-phase workflow guide)
   - `CONTRIBUTING.md` (contributor guidelines)
   - `.gemini/skills/UPDATE_CHECKLIST.md` (skill update checklist)
   - `.gitignore` (git exclusions)

   **Optionally copied verbatim:**
   - `.gemini/skills/agentdb-state-manager/` (if user selected yes)
   ...
   **Generated/adapted for new repo:**
   ...
   - `GEMINI.md` - Uses repo purpose, inserts workflow section from source
   ...
   **Created from scratch:**
   - `ARCHIVED/` directory with GEMINI.md and README.md
   - `planning/` directory with GEMINI.md, README.md, ARCHIVED/
   - `specs/` directory with GEMINI.md, README.md, ARCHIVED/
...
7. **Report and Next Steps:**
   ...
   - Provides next step commands:
     ...
     uv sync
     python .gemini/skills/bmad-planner/scripts/create_planning.py <feature> <gh-user>
     ```

**Key features:**
- Copies all 9 skills (including this meta-skill for future replication)
- Adapts README.md, GEMINI.md, pyproject.toml for new repo context
...
| Flag | Behavior |
|------|----------|
| (none) | Prompts if `.gemini/` exists |
| `--force` | Deletes existing `.gemini/` entirely, copies fresh |
...
2. **Call the script (don't reproduce functionality):**
   ...
   result = subprocess.run([
       'python',
       '.gemini/skills/initialize-repository/scripts/initialize_repository.py',
...
```
Gemini reads WORKFLOW.md                     ~500 tokens
Gemini reads GEMINI.md                       ~300 tokens
Gemini reads all skill SKILL.md files      ~1,200 tokens
Gemini manually copies files                 ~800 tokens
Gemini manually adapts documentation         ~400 tokens
Gemini manually creates git structure        ~300 tokens
                                           ___________
Total:                                     ~3,500 tokens
```
...
**After callable tool:**
```
Gemini invokes script                        ~150 tokens
Script handles all logic                        0 tokens (no Gemini context)
                                           ___________
Total:                                       ~150 tokens
```
...
```bash
# Copy only .gemini/skills/ directory
...
# Manually integrate pyproject.toml, README.md, GEMINI.md
...
- .gemini/skills/UPDATE_CHECKLIST.md (copied verbatim) - Skill update checklist

**Generated files (adapted for new repo):**
- README.md - Uses repo name, purpose, description from Q&A
- GEMINI.md - Uses repo purpose, inserts workflow section
...
**Directory structure:**
- ARCHIVED/ - With GEMINI.md, README.md
- planning/ - With GEMINI.md, README.md, ARCHIVED/
- specs/ - With GEMINI.md, README.md, ARCHIVED/
...
- .gemini/skills/agentdb-state-manager/ - DuckDB state tracking
...
[... complete workflow section copied from source GEMINI.md ...]
```
...
├── GEMINI.md                    # Generated for new repo
...
Created:
  ...
  - Documentation (WORKFLOW.md, CONTRIBUTING.md, README.md, GEMINI.md)
...
Next steps:
  ...
  2. uv sync
  3. uv run pytest
  4. Start Phase 1: python .gemini/skills/bmad-planner/scripts/create_planning.py <feature> stharrold
...
5. After initialization, review and customize:
   ...
   - GEMINI.md (add domain-specific guidance)
...
**COPY_VERBATIM:** ['.gemini/skills/', 'WORKFLOW.md', 'CONTRIBUTING.md', '.gitignore']
- **Rationale:** Workflow system is tech-agnostic and standardized. No customization needed.

**ADAPT_FILES:** ['README.md', 'GEMINI.md', 'pyproject.toml', 'CHANGELOG.md', 'TODO.md']
...
```
✗ Error: Source repository does not have workflow system
  Expected: ≥3/9 skills in .gemini/skills/
  Found: 0 skills
...
| Read GEMINI.md | 300 | Understanding repository context |
| Read all SKILL.md files | 1,200 | Understanding 9 skills |
| Manually copy files | 800 | Copy commands, validation |
| Manually adapt docs | 400 | Customize README, GEMINI.md, pyproject.toml |
...
| Script execution | 0 | No Gemini context needed |
